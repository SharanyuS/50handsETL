...
browser.get("https://data.oecd.org/price/inflation-cpi.htm")
print('ok')
time.sleep(3)
...
tiny_file_rename("raw_us_inflation_rate", "/Users/shricharanks108/PycharmProjects/50Hands_Sprint3/Sprint3_Part1")

with open('raw_us_inflation_rate.csv') as rf:
    csv_reader=csv.reader(rf)
    # next(rf)
    with open('us_inflation_rate.csv', 'w', newline='') as wf:
...
df=pd.read_csv('us_inflation_rate.csv')


df["SUBJECT"].replace({"ENRG": "Energy", "TOT": "Total","TOT_FOODENRG":"Total w/o energy and food"}, inplace=True)
df["MEASURE"].replace({"AGRWTH":"Annual Growth","IDX2015":"Index 2015"}, inplace=True)
df["FREQUENCY"].replace({"A":"Yearly","M":"Monthly","Q":"Quarterly"}, inplace=True)
df["INDICATOR"].replace({"CPI":"Consumer Price Index"},inplace=True)

df = df[df.LOCATION == 'USA']

df.to_csv('us_inflation_rate.csv',index=False)

upload_to_SQL('us_inflation_rate.csv', 8, "s3_us_inflation_rate")  
os.remove('raw_us_inflation_rate.csv')  