...
browser.get("https://data.oecd.org/hha/household-disposable-income.htm")
print('ok')
time.sleep(3)
...
tiny_file_rename("raw_us_household_disposable_income.csv", "/Users/shricharanks108/PycharmProjects/50Hands_Sprint3/Sprint3_Part1")


with open('raw_us_household_disposable_income.csv') as rf:
    csv_reader=csv.reader(rf)
    # next(rf)
    with open('us_household_disposable_income.csv', 'w', newline='') as wf:
...
df=pd.read_csv('us_household_disposable_income.csv')
#df['MEASURE'] = df['MEASURE'].apply(lambda x: "% of Household Disposable Income")
df['FREQUENCY'] = df['FREQUENCY'].apply(lambda x: "YEARLY")
df["SUBJECT"].replace({"NET": "NET(%)", "GROSSADJ": "GROSS ADJUSTED($)"}, inplace=True)
df['INDICATOR']=df['INDICATOR'].apply(lambda x:"Household Disposable Income")
df["MEASURE"].replace({"AGRWTH": "ANNUAL GROWTH(%)", "USD_CAP": "USD_per_capita($)"}, inplace=True)
df.to_csv('us_household_disposable_income.csv',index=False)

           

upload_to_SQL('us_household_disposable_income.csv', 8, "us_household_disposable_income") 
os.remove('raw_us_household_disposable_income.csv')   